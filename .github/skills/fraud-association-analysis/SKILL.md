...
[数据组装] 按案件号/保单号查询 ODPS，拉取保单传播图和关联案件
  → 详见 references/02_db_schema.md
        │
        ▼ Stage 2
[图计算] MO评分 → 聚类 → Leiden子社群 → 基础报告（含Section 4占位符）
  → 见下方"Stage 2 详细"
        │
        ▼ Stage 3
[智能解读] Claude 读取报告 + fraud_hypothesis → 生成解读内容 → 回写报告
...
生成结构化中文 Markdown 报告，包含 **Section 1-3（纯数据层，可复现）** + **Section 4 占位符**：

| 段落                   | 内容                                                                       | 来源     |
| ---------------------- | -------------------------------------------------------------------------- | -------- |
| **一、聚集性结论**     | 风险等级、相似社群数、可疑案件数、最高团伙分                               | 代码生成 |
| **二、社群明细**       | 每个社群：规模/关联类型/桥接节点/可疑案件列表（MO分/医院/疾病/天数/金额） | 代码生成 |
| **三、传播概览**       | 总保单/案件/赔付、分层统计、关联类型分布、MO相似度分布                     | 代码生成 |
| **四、智能解读结论**   | `<!--INTERPRETATION_PLACEHOLDER-->` 标记，待 Stage 3 回写                  | LLM填充  |
...
## Stage 3 详细（LLM 智能解读 + 回写报告）

### 3-1：读取材料

Claude 需要读取以下内容后再开始解读：
- `output/report.md`（Section 1-3 数据层）
- Stage 0 产出的 `fraud_hypothesis`（欺诈假设，用于验证比对）
- 如有需要，读取 `cluster_output.json` 中各社群的 `桥接节点列表` 和 `关联类型分布`

**解读规范**：详见 [references/04_interpret.md](references/04_interpret.md)

### 3-2：生成 Section 4 内容

按 `04_interpret.md` 中的 Prompt 模板完成四项任务：
1. 欺诈假设验证（高度吻合 / 部分吻合 / 不吻合）
2. 各社群关联机制解读（关联推断 + 置信度 + 支撑证据）
3. 跨社群共性信号扫描
4. P0/P1/P2 调查建议（具体到保单号/机构名）

### 3-3：回写报告（关键步骤）

解读完成后，**必须将内容写入报告文件**，替换占位符标记：

- 定位 `output/report.md` 中的以下内容：
  ```
  <!--INTERPRETATION_PLACEHOLDER-->
  ```
- 将该行替换为完整的 Section 4 Markdown 内容

**替换后效果**：`report.md` 中 Section 4 由占位符变为完整解读，整份报告（Section 1-4）成为最终交付物。

> **设计原则**：Section 1-3 由代码生成，客观、可复现；Section 4 由 LLM 生成，带置信度标注。
> 两层合并进同一文件，审计时可通过提交历史区分数据层与解读层的修改来源。
...
| `scripts/generate_report.py`     | 结构化报告生成（Section 1-3 数据层 + Section 4 占位符） |
...
- **回写报告时只替换 `<!--INTERPRETATION_PLACEHOLDER-->` 标记行，不修改 Section 1-3 内容**
